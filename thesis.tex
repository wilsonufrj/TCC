%% Copyright 2023 P. S. Eduardo.
%
% This work may be distributed and/or modified under the
% conditions of the LaTeX Project Public License, either version 1.3
% of this license or (at your option) any later version.
% 
% The Current Maintainer of this work is P. S. Eduardo.
%
% This work consists of the file poli.cls.
%% -----------------------------------------------------------------
% Escola Politécnica UFRJ LaTeX Template
% Version: 2302
% Author: Eduardo Paiva dos Santos
% email: eduardopaiva@poli.ufrj.br
% Base: CoppeTeX 2.3
%%------------------------------------------------------------------
\documentclass[grad,pdftex]{poli}
\usepackage[utf8]{inputenc}
\usepackage{amsmath,amssymb}
\usepackage{multirow}
\usepackage{longtable}
\usepackage{tikz}
\usetikzlibrary{shapes,arrows,chains,positioning}
\usepackage{enumitem}
\usepackage{indentfirst}
\usepackage{array}
\usepackage{diagbox}
\usepackage[numbers,sort&compress]{natbib}
\usepackage{mciteplus}

\usepackage{listings}

%\usepackage[alf, bibjustif]{abntex2cite}

\lstset{
    basicstyle=\ttfamily\small,
    numbers=left,
    numberstyle=\tiny\color{gray},
    keywordstyle=\color{blue},
    commentstyle=\color{green!50!black},
    stringstyle=\color{orange},
    breaklines=true,
    frame=single,
    showstringspaces=false,
    tabsize=4
}

\lstdefinelanguage{TypeScript}{
    sensitive=true,
    morekeywords={
        async,await,break,case,catch,class,const,continue,default,
        delete,do,else,export,extends,false,finally,for,function,
        if,import,in,instanceof,interface,let,new,null,number,
        private,public,readonly,return,string,super,switch,this,
        throw,true,try,type,typeof,undefined,var,void,while
    },
    morecomment=[l]{//},
    morecomment=[s]{/*}{*/},
    morestring=[b]',
    morestring=[b]"
}

\renewcommand{\lstlistingname}{Código:}
\renewcommand{\lstlistlistingname}{Lista de Códigos}

\makeatletter
\newcommand{\listofcodes}{%
    \if@twocolumn
        \@restonecoltrue\onecolumn
    \else
        \@restonecolfalse
    \fi
    \chapter*{\lstlistlistingname}%
    \addcontentsline{toc}{chapter}{\lstlistlistingname}%
    \@mkboth{\MakeUppercase\lstlistlistingname}%
        {\MakeUppercase\lstlistlistingname}%
    \@starttoc{lol}%
    \if@restonecol\twocolumn\fi
}
\makeatother

\makelosymbols
\postapproval{\input{Textual/Preambulo}}

\begin{document}
  \title{Biribá: Sistema web para a gestão do Programa de Aquisição de Alimentos}
  \foreigntitle{Biribá: Web System for Managing the Food Acquisition Program}
  \author{Wilson}{Albuquerque Ramos}
  \advisor{}{Celso Alexandre}{Souza de Alvear}{D.Sc.}
  %\coadvisor{Prof.}{Nome}{Sobrenome}{D.Sc.}
  \examiner{}{Flávio Luis de Mello}{D.Sc.}
  \examiner{}{Felipe Dias de Rezende Machado}{M.Sc.}
  %\examiner{Prof.}{Nome Completo}{D.Sc.}
  \department{DEL} %UTTILIZE A SIGLA DO SEU DEPARTAMENTO MODIFICAR OS NOMES DE CURSO, DEPARTAMENTO E OBTENÇÃO DE GRAU (Obs: caso tenha algum equívoco nesses argumentos, necessário modificar o arquivo poli.cls no local que faz a leitura deste argumento)
  \date{8}{2026}
  \keyword{Tecnologia social}
  \keyword{Software Livre}
  \keyword{Gestão comunitária}
  \keyword{Engenharia de software}
  \maketitle

  \frontmatter
  % \include{Pre-textual/dedic}
  \include{Pre-textual/thanks}
  \include{Pre-textual/resumo}
  \include{Pre-textual/abstract}
  \include{Pre-textual/siglas}
  \tableofcontents
  \listoffigures
  \listofcodes
  \printlosymbols

  \mainmatter
    \include{Textual/chapter01}
    \include{Textual/chapter03}
    % \include{Textual/chapter04}
    \include{Textual/chapter02}    
    \include{Textual/chapter05}
    % \include{Textual/chapter06}
    \include{Textual/chapter07}
  \backmatter
  \bibliographystyle{IEEEtranM}
  \bibliography{thesis}
  \appendix
  \include{Pos-textual/appenA}
\end{document}
