...
\begin{figure}[H]
    ...
    \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
        component/.style={draw, rounded corners, align=center, minimum width=3.1cm, minimum height=1.15cm, fill=blue!5},
        deploy/.style={draw, rounded corners, align=center, minimum width=3.1cm, minimum height=1.15cm, fill=green!8},
        flow/.style={->, >=stealth, thick}
    ]
        \node[component] (browser) {Usuários\\Navegador web};
        \node[component, right=1.2cm of browser] (frontend) {Frontend\\React 19 e TypeScript};
        \node[component, right=1.2cm of frontend] (backend) {Backend\\Java 17 e Spring Boot 3};
        \node[component, right=1.2cm of backend] (database) {Banco de dados\\PostgreSQL};

        \draw[flow] (browser) -- node[above]{HTTPS} (frontend);
        \draw[flow] (frontend) -- node[above]{API REST/JSON} (backend);
        \draw[flow] (backend) -- node[above]{JPA/Hibernate} (database);

        \node[deploy, below=1.4cm of frontend] (gitlab) {GitLab\\CI e CD};
        \node[deploy, below=1.4cm of backend] (dockerhub) {Docker Hub\\Imagens dos módulos};
        \node[deploy, below=1.4cm of database] (vps) {VPS de homologação\\Docker Compose};

        \draw[flow] (gitlab) -- node[above]{build} (dockerhub);
        \draw[flow] (dockerhub) -- node[above]{implantação} (vps);
    \end{tikzpicture}%
    }
    \caption{Arquitetura lógica e fluxo de implantação do sistema Biribá}
